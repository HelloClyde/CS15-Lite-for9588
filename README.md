...
从旧版升级时，请先删除真机上的旧
`A:\应用\数据\CS15LITE\CS15.C15PAK`，再复制新版文件。M15 的七地图资源包约
11 MB；直接覆盖旧包可能因 9588 的 FAT 空闲空间碎片化而复制失败。
...
| 贴图 arena | 160,000 B；菜单背景与地图分时复用，地图峰值 154,848 B（Office） |
| 模型 arena | 64 KiB；最大组合估算约 63,008 B |
| 资源/音频共享 scratch | 2 KiB |
| 完整静态镜像 | 当前 655,448 B；构建时强制检查 1,572,864 B 上限 |

世界贴图在 PC 上预先选择最大 32 像素 mip 并量化到 64 色；玩家皮肤预滤波为
16×16；900 项以上资源目录在磁盘上按 ID 排序并二分查询，BSP 的 PVS 按需流式读取，
动画只流入当前顶点帧，音频按 512-byte 资源块读取并输出为 1024-byte
固件块，不常驻整段 PCM。
