...
- **三张经典地图**：`de_dust2`、`cs_assault`、`cs_italy`；不再打包
  `de_dust`，并保留 BSP/PVS、纹理、hull 碰撞、重力、台阶和墙体滑动。
...
- **可调 AI**：难度实际改变反应时间、连发节奏、命中率、伤害和战斗走位，
  不是只有菜单文字变化。
- **炸弹回合**：`de_dust2` 从 BSP 实体提取 A/B 点；T 携带、安装和引爆 C4，
  CT 可拆弹，Bot 也会执行对应目标，回合判定会区分已安装/未安装状态。
- **计分板**：按住右侧 `SCORE` 显示玩家和 7 个 Bot 的阵营、击杀、死亡及
  T/CT 回合比分。
- **完整游戏流程**：经典主菜单、地图与 T/CT 选择、分页购买、回合胜负、
  金钱、生命、弹药和队伍存活数。
...
| 区域 | 上限 / 当前峰值 |
|---|---:|
| 地图 arena | 864,000 B；当前峰值 863,632 B（`cs_italy`） |
| 贴图 arena | 86,000 B；当前峰值 84,032 B（`cs_assault`） |
| 模型 arena | 64 KiB；最大组合估算约 63,008 B |
| 资源/音频共享 scratch | 2 KiB |
| 完整静态镜像 | 1,562,420 / 1,572,864 B |

世界贴图在 PC 上预先选择最大 32 像素 mip 并量化到 64 色；玩家皮肤预滤波为
16×16；565 项资源目录在磁盘上按 ID 排序并二分查询，BSP 的 PVS 按需流式读取，
